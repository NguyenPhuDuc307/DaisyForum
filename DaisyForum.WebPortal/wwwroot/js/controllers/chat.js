$(document).ready(function () {
    var connection = new signalR.HubConnectionBuilder().withUrl("https://localhost:5000/chatHub").build();
    ...
    function AppViewModel() {
        ...
        self.deleteRoom = function () {
            fetch('/Chat/DeleteRoom/' + self.joinedRoom().id(), {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
        ...
        self.messageHistory = function () {
            fetch('/Chat/GetMessages/Room/' + viewModel.joinedRoom().name())
                .then(response => response.json())
                .then(data => {
